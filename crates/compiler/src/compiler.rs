        ...
        let tests = vec![
            CompilerTestCase {
                ...
                expected_instructions: vec![
                    ...
                    Instructions(make(Opcode::OpConstant, &[4])),
                    Instructions(make(Opcode::OpAdd, &[])),
                    Instructions(make(Opcode::OpIndex, &[])),
                    Instructions(make(Opcode::OpPop, &[])),
                ],
            },
            CompilerTestCase {
                ...
                expected_instructions: vec![
                    Instructions(make(Opcode::OpConstant, &[0])),
                    Instructions(make(Opcode::OpConstant, &[1])),
                    Instructions(make(Opcode::OpHash, &[2])),
                    Instructions(make(Opcode::OpConstant, &[2])),
                    Instructions(make(Opcode::OpConstant, &[3])),
                    Instructions(make(Opcode::OpSub, &[])),
                    Instructions(make(Opcode::OpIndex, &[])),
    ...
    fn test_functions() {
        let tests = vec![
            CompilerTestCase {
                ...
                expected_constants: vec![
                    ...
                    ExpectedConstant::Instructions(vec![
                        ...
                        Instructions(make(Opcode::OpConstant, &[1])),
                        Instructions(make(Opcode::OpAdd, &[])),
                        Instructions(make(Opcode::OpReturnValue, &[])),
                    ]),
                ],
                expected_instructions: vec![
                    Instructions(make(Opcode::OpConstant, &[2])),
            ...
            CompilerTestCase {
                ...
                expected_constants: vec![ExpectedConstant::Instructions(vec![Instructions(make(
                    Opcode::OpReturn,
                    &[],
                ))])],
                expected_instructions: vec![
        ...
        for (idx, constant) in expected.into_iter().enumerate() {
            match constant {
                ...
                ExpectedConstant::Instructions(items) => {
                    ...
                    test_instructions(items, code::Instructions(compiled_fn.instructions.clone()));
                }
            }
        }
    }
